...
class DeepRVFL(object):
    def __init__(self, Nu,Nh,Nl, configs, verbose=0,act='sigmoid'):
    ...
    def computeLayerState(self,  layer,x_raw,inistate=None, DeepIP = 0):
        ...
        if layer==0:
            ...
            if self.act=='sigmoid':
                ...
                state=relu(self.Win[layer][:,:-1].dot(x_raw))+np.tile(np.expand_dims(self.Win[layer][:,-1],1),[1,Ns])
        else:
            ...
            if self.act=='sigmoid':
                ...
                state=relu(self.Win[layer][:,:-1].dot(x_cat))+np.tile(np.expand_dims(self.Win[layer][:,-1],1),[1,Ns])
        return state
