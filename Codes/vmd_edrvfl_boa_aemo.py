...
def dRVFL_predict(hyper,norm_data,train_idx,test_idx,layer,s,last_states=None,dec_=None,alldecs=None):
    ...
    data=Struct()
    if dec_ is None:
        dec_=alldecs[str(w)+str(k)]#decomposer.ts_walkforward(norm_data.ravel(),w,k,'vmd',pad_l=0,pad_raw=True)
...
def cross_validation(hypers,data,raw_data,train_idx,val_idx,Nl,regs,input_scale,ratios=[1],scaler=None,s=0,boat=50):
    ...
    for w in [48,96,144]:
        for k in [2,3,4]:
            dec_=decomposer.ts_walkforward(data.ravel(),w,k,'vmd',pad_l=0,pad_raw=True)
            alldecs[str(w)+str(k)]=dec_
    for i in range(Nl):
        # print(i,layer_s)
        layer=i+1
        # print(i,dec_s is None)
        layer_h,layer_s,dec_s=layer_cross_validation(hypers,data,raw_data,train_idx,val_idx,layer,
...
def layer_cross_validation(hypers,data,raw_data,train_idx,val_idx,layer,
    ...
    args=fmin(fn=layer_obj,
                ...
                algo=tpe.suggest)
    if layer==1:
        #vmdRVFL INPUT SCALE 1
        wi=args['w']
        ki=args['k']
        issi=args['input_scale']
        # print(wi,ki)
        best_hyper=[args['Nhs'],args['regs'],issi,[48,96,144][wi],[2,3,4][ki]]#,args['ratios']]
    else:
        issi=args['input_scale']
        best_hyper=[best_hidden[0],args['regs'],issi,best_w[0],best_k[0]]#,args['ratios']]
    if layer>1:
...
def layer_obj(args):
    ...
    # print(args['1w'])
    hyper=[args['Nhs'],args['regs'],args['input_scale'],args['w'],args['k']]#,args['ratios']]
    data=args['data']
    ...
    for co in countrys:
        for month_ in  ['01_','04_','07_','10_']:
            ...
            for s in np.arange(seeds):
                ...
                # decomposer.ts_walkforward(x, w, k, 'vmd',pad_raw=True)
                ...
                norm_data=scaler.transform(data_)
                w,k=ed_best_hypers[0][3],ed_best_hypers[0][4]
            
                dec_=decomposer.ts_walkforward(norm_data.ravel(),w,k,'vmd',pad_l=0,pad_raw=True)
            ...
            dfea.to_csv('D:\\DeepESN-master\\AEMO\\vmdRVFLBOA'+str(boat)+name)
